...
while IFS=, read -r sample_name processed_path needs_reprocessing || [ -n "$sample_name" ]; do
    ...
    echo "[INFO] Creating a copy of the $sample_name inside the proteins directory:"

    cp ${OUTPUT_DIR}/${sample_name}/*processed.pdb ${DATA_DIR}/processed/proteins/

    echo "[INFO] Copy created successfully."
